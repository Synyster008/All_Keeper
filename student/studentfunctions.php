<?php
  ...
  function getStoreRequest($roll, $db){
    $query_request_count = "Select count(*) from storerequest where rollnumber='$roll'";
    $result_request_count = mysqli_query($db, $query_request_count);
    if (mysqli_num_rows($result_request_count) == 1) {
      $countRow = mysqli_fetch_assoc($result_request_count);
    }
  ...
  function getReport($roll, $db){
    ...
    // Get Number Of Request, Delivery Guy Info
  function getData($roll, $db){
    $query_request = "Select sr.deliveryguy_id as wid,sr.storerequest_id ,sr.reqdate, sr.requesttime,sr.req_status,sr.delivery, s.room ,dg.name from storerequest sr Inner Join student s on s.rollnumber=sr.rollnumber Left Join deliveryguy dg on sr.deliveryguy_id=dg.deliveryguy_id  where sr.rollnumber='$roll' order by sr.reqdate desc";
    $result_request = mysqli_query($db, $query_request);
    return $result_request;
  }

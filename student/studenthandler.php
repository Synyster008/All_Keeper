<?php
  ...
  // ================== Stationery Request Handler =================== //
  if(isset($_POST['stationerySubmit']) && isset($_SESSION['rollnumber'])){
    $rollnumber = $_SESSION['rollnumber'];
    $reqDate = mysqli_real_escape_string($db, $_POST['reqDate']);
    $reqTime = mysqli_real_escape_string($db, $_POST['reqTime']);
    $reqpencil = mysqli_real_escape_string($db, $_POST['reqpencil']);
    $reqpen = mysqli_real_escape_string($db, $_POST['reqpen']);
    $reqeraser = mysqli_real_escape_string($db, $_POST['reqeraser']);
    $reqnotebook = mysqli_real_escape_string($db, $_POST['reqnotebook']);
    $reqsharpner = mysqli_real_escape_string($db, $_POST['reqsharpner']);
    $reqcalculator = mysqli_real_escape_string($db, $_POST['reqcalculator']);
    $reqpapers = mysqli_real_escape_string($db, $_POST['reqpapers']);
    ...
    $req_query = "INSERT into storerequest(rollnumber,reqdate,requesttime,reqpencil,reqpen,reqeraser,reqnotebook,reqsharpner,reqcalculator,reqpapers,delivery) values ('$rollnumber','$reqdate','$reqTime','$reqpencil','$reqpen','$reqeraser','$reqnotebook','$reqsharpner','$reqcalculator','$reqpapers','$delivery')";
    echo $req_query;
    $req_result = mysqli_query($db,$req_query);
    if ($req_result) {
      $_SESSION['req_sent'] = "Stationery Request is sent";
    }else {
      $_SESSION['req_failed'] = "Failed to send request, contact administrator.";
    }
    header("Location: requestStorekeeper.php");
    }
      else{
        $_SESSION['req_err'] = "Please enter future dates only.";
        header("Location: requestStorekeeper.php");
